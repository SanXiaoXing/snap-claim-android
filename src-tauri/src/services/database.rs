...
impl Database {
    /// 打开或创建数据库（自动建表）
    /// ponytail: 移动端改用 app_data_dir()/snap_claim.db，无 exe_dir 概念。
    /// desktop 版用 current_exe().parent()/data/db，Android 上无此路径。
    pub fn open(app: &AppHandle) -> Result<Self, crate::error::AppError> {
    ...
    pub fn get_history_detail(&self, id: i64) -> Result<HistoryDetail, crate::error::AppError> {
        ...
        let (name, created_at, start_date, end_date, days, totals_json, preview_rows_json, remark) = conn.query_row(
            ...
            |row| {
                ...
                let remark: Option<String> = row.get(7)?;
                Ok((name, created_at, start_date, end_date, days as u32, totals_json, preview_rows_json, remark))
            },
        )?;
    ...
    pub fn delete_history(&self, id: i64) -> Result<(), crate::error::AppError> {
        ...
        if affected == 0 {
            return Err(crate::error::AppError::Database(format!("记录 ID {} 不存在", id)));
        }
        Ok(())
    }
}

/// 获取数据库文件路径：app_data_dir()/snap_claim.db
/// ponytail: 移动端用 Tauri path API，Android 落到 /data/data/<pkg>/files/snap_claim.db。
fn get_db_path(app: &AppHandle) -> Result<String, crate::error::AppError> {
    let app_data = app
        .path()
        .app_data_dir()
        .map_err(|e| crate::error::AppError::Database(format!("获取 app_data_dir 失败: {e}")))?;
    std::fs::create_dir_all(&app_data)?;
    let db_path = app_data.join("snap_claim.db");
