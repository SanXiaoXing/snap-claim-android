...
- 所有单据和明细存在手机本地 SQLite，**不会因为记账功能上传任何服务器**
- 没有账号、没有注册、不收集你的个人信息
- 相机权限只用来扫码，票据识别全程在你手机上完成
- **AI 情况说明（可选）**：仅当你在 AI 弹层里点「生成」，才会把你输入的原因文字，以及本机保存的智谱 API Key，用于请求**你自己配置的**模型服务（默认智谱）。App 不自建服务器、不代管 Key。Key 存在本机，并会随「导出数据」写入 `.snapbackup`，请妥善保管备份文件
- 其余联网：你自己点「关于」页的反馈按钮去 GitHub 提 issue
...
**Q：需要联网吗？**
A：记账、识别、备份不需要联网。只有你主动使用「AI 情况说明」并点生成时，才会访问你配置的智谱接口；不使用 AI 则不会因此联网。
...
| AI | http + 智谱 OpenAI 兼容 API | 「情况说明」流式生成（用户自备 Key） |
